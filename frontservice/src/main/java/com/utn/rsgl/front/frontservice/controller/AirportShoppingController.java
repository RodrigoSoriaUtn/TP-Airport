	...
	public ResponseEntity<List<AirportResponse>> getAirports(HttpServletRequest request){
		...
		//if(Objects.nonNull(session.getAttribute("user"))){
			try {
				...
				List<AirportDTO> airports = (List<AirportDTO>) consumingAirports.getEntity(AirportDTO.class, ip);
		...
		if(Objects.nonNull(session.getAttribute("user"))){
			if(Objects.nonNull(iata)){
				try {
					...
					routes = (List<RouteDTO>) consumingAirports.getEntity(RouteDTO.class, ip);
		...
		if(Objects.nonNull(session.getAttribute("user"))){
			if(Objects.nonNull(routeRequest)){
				try {
					...
					List<PricePerCabinPerRouteDTO> cabinsAndprices = (List<PricePerCabinPerRouteDTO>) consumingAirports.getEntity(PricePerCabinPerRouteDTO.class, ip.concat(parameters));
